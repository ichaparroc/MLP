	...
	for(int epoch=0;epoch<n_epoch;epoch++)
	{
		...
		for(int instance=0;instance<n_images_train;instance++)
		{
			...
			for(int i=0;i<layer[n_layers-1];i++)
            	instance_error+=0.5*pow(output_train[instance][i]-a[n_layers-1][i],2);
			...
			{
				d_b[m][i]=(a[m+1][i]-output_train[instance][i])*a[m+1][i]*(1-a[m+1][i]);
			...
			for(m=0;m<n_layers-1;m++)
				for(int j=0;j<layer[m+1];j++)
				{
					...
					for(int i=0;i<layer[m];i++)
					{
						...
						if(DBG) cout<<endl<<"d_w["<<m<<"]["<<i<<"]["<<j<<"] = "<<d_b[m][j]*a[m][i];
		...
		//for(int instance=0;instance<1;instance++)
		{
			...
			instance_error=0;
			for(int i=0;i<layer[n_layers-1];i++)
            	instance_error+=0.5*pow(output_test[instance][i]-a[n_layers-1][i],2);
