...
//#pragma omp parallel for num_threads(THREAD_COUNT) private(local_a,real,max,calculated,instance_error,aux,z) shared(input_train,n_images_train,n_layers,layer,b,w,output_train,accuracy_train,error_train,d_b,d_w)
//#pragma omp parallel for num_threads(THREAD_COUNT) private(real,max,calculated,instance_error,aux,z) shared(input_train,n_images_train,n_layers,layer,b,w,output_train,accuracy_train,error_train,d_b,d_w) reduction(+:accuracy_train,error_train,d_b,d_w)
#pragma omp parallel for num_threads(THREAD_COUNT) private(real,max,calculated,instance_error,aux,z) shared(input_train,n_images_train,n_layers,layer,b,w,output_train) reduction(+:accuracy_train,error_train)
		for(int instance=0;instance<n_images_train;instance++)
		{
			...
			if(real==calculated)
//#pragma omp critical(accuracy_train)
				accuracy_train++;
...
//#pragma omp critical(error_train)
			...
			{
				aux=derror(output_train[instance][i],local_a[m+1][i])*dact(local_a[m+1][i]);
#pragma omp critical(d_b1)
				d_b[m][i]+=aux;
				for(int j=0;j<layer[m];j++)
#pragma omp critical(d_w1)
			...
			for(int m=n_layers-3;m>-1;m--)
				for(int i=0;i<layer[m+1];i++)
				{
					...
					aux=z*dact(local_a[m+1][i]);
#pragma omp critical(d_b2)
					d_b[m][i]+=aux;
					for(int j=0;j<layer[m];j++)
#pragma omp critical(d_w2)
						d_w[m][j][i]+=aux*local_a[m][j];
					//if(DBG) cout<<endl<<"d_b["<<m<<"]["<<i<<"] = "<<d_b[m][i];
				}
			//for(int i=0;i<n_layers;i++)
